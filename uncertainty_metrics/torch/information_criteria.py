...
def ensemble_cross_entropy(labels, logits, binary=False, aggregate=True):
    ...
    if binary:
        ce = F.cross_entropy(
            target=labels.unsqueeze(0).repeat(logits.size(0), 1),
            input=logits
        )
    else:
...
def gibbs_cross_entropy(labels, logits, binary=False, aggregate=True):
    ...
    if binary:
        nll = F.cross_entropy(
            # labels=tf.broadcast_to(labels.unsqueeze(0), tf.shape(logits)),
            target=labels.unsqueeze(0).repeat(logits.size(0), 1),
            input=logits
        )
    else:
        labels = labels.type(torch.int32)
        nll = F.cross_entropy(
            # labels=tf.broadcast_to(labels.unsqueeze(0), logits.shape[:-1]),
            target=labels.unsqueeze(0).repeat(logits.size(0), 1),
            input=logits)
...
def negative_waic(logp, waic_type="waic1"):
    ...
    """
    logp_mean = torch.logsumexp(logp, 1) - math.log(int(logp.shape[1]))
    if waic_type == "waic1":
        _, logp_var = logp.std(dim=1)
        neg_waic = (logp_mean - logp_var).mean(0)
        neg_waic_var = (logp_mean - logp_var).std(0)
    elif waic_type == "waic2":
        gibbs_logp = torch.mean(logp, dim=1)
        neg_waic = (2.0 * gibbs_logp - logp_mean).mean(0)
        neg_waic_var = (2.0 * gibbs_logp - logp_mean).std(0)

    neg_waic_sem = (neg_waic_var / float(int(logp.shape[1]))).sqrt()
