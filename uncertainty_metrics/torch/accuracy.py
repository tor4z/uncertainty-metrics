...
from uncertainty_metrics.torch import calibration
...
def _bin_probabilities(num_bins, index, dtype):
    ...
    """
    return (torch.linspace(0.0 + 1.0 / num_bins, 1.0, num_bins)[index]).type(dtype)
...
class OracleCollaborativeAccuracy(calibration.ExpectedCalibrationError):
    ...
    def __init__(self, fraction=0.01, num_bins=100, name=None, dtype=None):
        ...
        self.collab_correct_sums = nn.Parameter(torch.zeros(num_bins),
                                                requires_grad=True)

    def result(self):
        """Computes the expected calibration error."""
        num_total_example = torch.sum(self.counts)
        num_oracle_examples = torch.tensor(int(num_total_example * self.fraction),
                                           dtype=self.dtype)
        ...
        for i in range(self.num_bins):
            ...
            if num_oracle_examples_so_far + cur_bin_counts <= num_oracle_examples:
                ...
                num_correct_oracle = num_oracle_examples - num_oracle_examples_so_far
        ...
        self.collab_correct_sums = torch.stack(collab_correct_sums)

        # non_empty = tf.math.not_equal(self.counts, 0)
        # counts = tf.boolean_mask(self.counts, non_empty)
        mask = self.counts != 0
        counts = self.counts[mask]
        collab_correct_sums = self.collab_correct_sums[mask]

        return collab_correct_sums.sum() / counts.sum()
