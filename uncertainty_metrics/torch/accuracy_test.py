...
class OracleCollaborativeAccuracyTest(parameterized.TestCase, unittest.TestCase):
    ...
    def testOracleCollaborativeAccuracy(self):
        ...
        metric = um.OracleCollaborativeAccuracy(
            fraction, num_bins, name='collab_acc', dtype=torch.float64)

        acc1 = metric(labels, pred_probs)
        self.assertAllClose(acc1, correct_acc)

        actual_bin_counts = convert_to_tensor(metric.counts)
        actual_bin_correct_sums = convert_to_tensor(metric.correct_sums)
        actual_bin_prob_sums = convert_to_tensor(metric.prob_sums)
        actual_bin_bin_collab_correct_sums = convert_to_tensor(
