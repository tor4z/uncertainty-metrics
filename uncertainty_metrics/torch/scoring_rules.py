...
def brier_decomposition(labels=None, logits=None, probabilities=None):
    ...
    * `Reliability`, a measure of calibration of predictions against the true
    ...
    pred_class = torch.argmax(probabilities, dim=1).type(torch.int32)
    confusion_matrix = metrics.confusion_matrix(pred_class, labels, labels=nlabels)
    confusion_matrix = torch.tensor(confusion_matrix).type(torch.int32)
    dist_weights = torch.sum(confusion_matrix, dim=1)
    dist_weights /= torch.sum(dist_weights)
    pbar = torch.sum(confusion_matrix, dim=0)
    pbar /= torch.sum(pbar)
    ...
    dist_mean = confusion_matrix / torch.unsqueeze(
        torch.sum(confusion_matrix, dim=1) + 1.0e-7, dim=1)

    # Uncertainty: quadratic entropy of the average label distribution
    uncertainty = -torch.sum(pbar.square())

    # Resolution: expected quadratic divergence of predictive to mean
    resolution = (torch.unsqueeze(pbar, dim=1) - dist_mean).suqare()
    resolution = torch.sum(dist_weights * torch.sum(resolution, dim=1))

    # Reliability: expected quadratic divergence of predictive to true
    prob_true = torch.gather(dist_mean, index=pred_class, dim=0)
    reliability = torch.sum((prob_true - probabilities).square(), dim=1)
...
def brier_score(labels=None, logits=None, probabilities=None, aggregate=True):
    ...
    if (probabilities.get_shape().as_list()[-1] == 1 or
        ...
        raise ValueError(
            ...
            " shape (n, 2) for nlabels=2 but got ", probabilities.get_shape())
    _, nlabels = probabilities.shape  # Implicit rank check.
    plabel = torch.sum(one_hot(labels, nlabels) * probabilities, dim=1)

    brier = torch.sum(probabilities.square(), dim=1) - 2.0 * plabel
    if aggregate:
...
def crps_normal_score(labels=None, means=None, stddevs=None):
    ...
    dist = td.normal.Normal(loc=0.0, scale=1.0)
    labels_std = (labels - means) / stddevs

    crps = 2.0 * dist.log_prob(labels_std).exp() +\
           labels_std * (2.0 * dist.cdf(labels_std) - 1.0)
...
def one_hot(labels, nlabels):
    output = torch.zeros((labels.shape[0], nlabels))
    output.scatter_(1, labels, 1)
    return output
